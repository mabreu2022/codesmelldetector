...
(*$HPPEMIT END '#if defined(XML_OMNIXMLDOM_PRAGMA_INIT_OMNIUNITINITIALIZATION)' *)
...
{ IOXMLDOMNodeRef }
 ...
 TODOMNode = class(TODOMInterface, IOXMLDOMNodeRef, IDOMNode, IDOMNodeEx, IDOMNodeSelect)
  ...
  private class var
    FSharedIDOMImpl: IDOMImplementation;
    FSharedDOMImpl: TODOMImplementation;
  protected
    class property SharedIDOMImpl: IDOMImplementation read FSharedIDOMImpl;
    class property SharedDOMImpl: TODOMImplementation read FSharedDOMImpl;
  public
...
begin
  ...
  begin
    case NativeNode.NodeType of
      ...
      DOCUMENT_NODE: DnWrapper := TODOMDocument.Create(OmniXML4Factory.SharedDOMImpl, CreateXMLDoc, False);
...
begin
  if not Assigned(FOwnerDocument) then
    FOwnerDocument := TODOMDocument.Create(OmniXML4Factory.SharedDOMImpl, NativeNode.OwnerDocument, False);
...
begin
  if SharedIDOMImpl = nil then
  begin
    TMonitor.Enter(Self);
    try
      if SharedIDOMImpl = nil then
      begin
        FSharedDOMImpl := TODOMImplementation.Create;
        FSharedIDOMImpl := FSharedDOMImpl as IDOMImplementation;
      end;
    finally
      TMonitor.Exit(Self);
    end;
  end;
  Result := SharedIDOMImpl;
end;
